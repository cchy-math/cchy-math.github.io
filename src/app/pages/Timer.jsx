...
export default function Timer() {
    ...
    useEffect(() => {
        let paperData;
        if (
            selectedSubject === "中國語文 Chinese Language" ||
            selectedSubject === "英國語文 English Language"
        ) {
            paperData = lang_papers;
        } else if (
            selectedSubject === "數學 Mathematics" ||
            selectedSubject === "數學必修部分 Mathematics Compulsory Part"
        ) {
            paperData = math_papers;
        } else if (
            selectedSubject ===
            "數學延伸部分單元(一) Mathematics Extended Part Modules 1"
        ) {
            paperData = m1_papers;
        } else if (
            selectedSubject ===
            "數學延伸部分單元(二) Mathematics Extended Part Modules 2"
        ) {
            paperData = m2_papers;
        } else if (selectedSubject === "視覺藝術 Visual Arts") {
            paperData = va_papers;
        } else if (
            (yearSelection.split(" ", 2)[1] === "S4" ||
                yearSelection.split(" ", 2)[1] === "S5" ||
                yearSelection.split(" ", 2)[1] === "S6") &&
            selectedSubject !==
                "公民與社會發展 Citizenship and Social Development" &&
            selectedSubject !== "視覺藝術 Visual Arts" &&
    ...
    function calculateEndTime(sH, sM, dH, dM) {
        let endTime =
            Number(sH) * 60 +
            Number(sM) +
            Number(dH.slice(0, dH.indexOf(" "))) * 60 +
            Number(dM.slice(0, dM.indexOf(" ")));
    ...
    for (let i = 0; i < 60; i += 5) {
        minuteOptions.push(<option key={i}>{i < 10 ? "0" + i : i}</option>);
    }
    ...
    useEffect(() => {
        setExamTime(
            startHour +
                ...
                startMin +
                " - " +
                calculateEndTime(startHour, startMin, durationHour, durationMin)
        );
    ...
    const handleStartClick = () => {
        ...
        interval = setInterval(() => {
            ...
            if (barWidth >= barWidth15 && barWidth <= 100) {
                document.querySelector(".time-left").className =
                    "progress-bar progress-bar-striped progress-bar-animated time-left bg-success";
                document.querySelector("#exam-time-left-info").className =
                    "col-xs-4 col-lg-8 display-5 text-success";
                document.querySelector(".progress.time-left-bar").className =
                    "progress time-left-bar time-left-bar-success flex-row-reverse my-3 p-0";
            }
            if (barWidth >= barWidth5 && barWidth < barWidth15) {
                document.querySelector(".time-left").className =
                    "progress-bar progress-bar-striped progress-bar-animated time-left bg-warning";
                document.querySelector("#exam-time-left-info").className =
                    "col-xs-4 col-lg-8 display-5 text-warning";
                document.querySelector(".time-left-bar").className =
                    "progress time-left-bar time-left-bar-warning flex-row-reverse my-3 p-0";
                setAnnouncement15(true);
            }
            if (barWidth >= 0 && barWidth < barWidth5) {
                document.querySelector(".time-left").className =
                    "progress-bar progress-bar-striped progress-bar-animated time-left bg-danger";
                document.querySelector("#exam-time-left-info").className =
                    "col-xs-4 col-lg-8 display-5 text-danger";
                document.querySelector(".time-left-bar").className =
                    "progress time-left-bar time-left-bar-danger flex-row-reverse my-3 p-0";
    ...
    useEffect(() => {
        let interval = setInterval(() => {
            ...
            if (
                autoStart &&
                Number(date.getHours()) === Number(startHour) &&
                Number(date.getMinutes()) === Number(startMin) &&
        ...
        <main className="container">
                    ...
                    <form className="" visible={formVisible}>
                        ...
                        <section className="p-3 justify-content-center">
                            <div className="row p-3 justify-content-center">
                                <div className="col-lg-1 col-xs-12 p-3">
                                    <label for="year" class="form-label h4">
                                        Year
                                    </label>
                                    <select
                                        value={yearSelection}
                                ...
                                <div className="col-lg-4 col-xs-12 p-3">
                                    <label for="subject" class="form-label h4">
                                        Subject
                                    </label>
                                    <select
                                ...
                                <div className="col-lg-3 col-xs-12 p-3">
                                    <label for="paper" class="form-label h4">
                                        Paper
                                    </label>
                                    <select
                                ...
                                <div className="col-lg-2 col-xs-12 p-3">
                                    <label for="display" class="form-label h4">
                                        Language
                                    </label>
                                    <select
                                    ...
                                    >
                                        <option></option>
                                        <option id="中文 Chinese">
                                            中文 Chinese
                                        </option>
                                        <option id="英文 English">
                                            英文 English
                                        </option>
                                        <option id="雙語顯示 Bilingual">
                                            雙語顯示 Bilingual
                                        </option>
                                    </select>
                                </div>
                        ...
                        <section className="p-3">
                            <div className="row p-3 justify-content-center">
                                <div className="col-lg-2 col-xs-12 p-3">
                                    ...
                                    <div className="row h3">
                                        <select
                                            ...
                                            class="col mx-3 form-select bg-dark text-light"
                                            id="starting-time-hour"
                                            required=""
                                        >
                                            <option></option>
                                            <option>08</option>
                                ...
                                <div className="col-lg-2 col-xs-12 p-3">
                                    <label for="duration" class="form-label h4">
                                        Duration Time
                                    </label>
                                    <div className="row h3">
                                        <select
                                ...
                                <div className="col-lg-2 col-xs-12 p-3">
                                    <label for="auto" class="form-label h4">
                                        Auto Start
                                    </label>
                                    <select
                                        onChange={handleStartingMethodChange}
                                        class="form-select bg-dark text-light"
                                        id="auto"
                                        required=""
                                    >
                                ...
                                <div className="col-lg-2 col-xs-12 px-4">
                                    <button
                                        ...
                                        className="button-rainbow my-5 h5"
                                        style={{ width: "100%" }}
                                    >
                                        Done
                                    </button>
            ...
            {displayVisible && (
                ...
                >
                    ...
                    <section
                        id="time-info-display"
                        className="vw-100 px-4 pb-3"
                    >
                        <section
                        ...
                        >
                            ...
                            {announcement15 &&
                                selectedLanguage === "英文 English" &&
                                "You have 15 minutes left. Make sure you have written your name, class and class number on the Question papers and the Answer Sheets."}
                            {announcement5 &&
                                selectedLanguage === "中文 Chinese" &&
                                "考試時間尚餘五分鐘。請考生確保在試卷及答題紙（或補充答題紙）上寫上姓名、班別及學號。將不必要的文字、草稿及圖形劃去。當考試完結，宣布「停止作答」後，考生不可再改動試卷及答題紙內的內容。"}
                            {announcement5 &&
                                selectedLanguage === "英文 English" &&
                                "You have 5 minutes left. Make sure you have written your name, class and class number on the Question papers and the Answer Sheets. Cross out all unwanted materials. You will NOT be allowed to work on your Question papers and the Answer Sheets after the 'Stop working' announcement."}
                            {announcement0 &&
                                selectedLanguage === "中文 Chinese" &&
                                `現在時間是${
                                    examTime.split(" - ")[1]
                                }。考試完結，停止作答。請放下所有文具。考生須安坐靜候，待得到指示後才可收拾個人物件。在離開試場前，切勿交談。將試卷及答題紙由後面傳上，傳卷時請將自己的答題紙放於其他考生的答題紙之上。`}
                            {announcement0 &&
                                selectedLanguage === "英文 English" &&
                                `The time now is ${
                                    examTime.split(" - ")[1]
                        ...
                        <section id="bar" className="row px-5 pb-5">
                            ...
                            <div className="row align-items-center">
                                ...
                                <div
                                    id="exam-time-left-info"
                                    className="col-xs-4 col-lg-8 display-5 text-info"
                                >
                                    {selectedLanguage === "中文 Chinese" &&
                                        "剩餘時間 "}
                                    {selectedLanguage === "英文 English" &&
